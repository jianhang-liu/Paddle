 ...
 public:
  void Compute(const paddle::framework::ExecutionContext& ctx) const override {
    ...
    if (out_var->IsType<LoDTensor>()) {
      ...
      if (src_tz.size() == 1 &&
          (input_format == memory::format::nchw ||
	...
	auto src_mem = memory(src_mpd, to_void_cast(input_data));
      ...
      auto dst_md = memory::desc(dst_tz, memory::data_type::f32,
                                 memory::format::any);
      ...
      if (in_place)
	// allocate additional memory. MKLDNN only support out-of-place sum
	dst_mem.reset(new memory(sum_pd.dst_primitive_desc()));
      ...
      // finally create sum primitive
      auto sum_prim = mkldnn::sum(sum_pd, inputs, *dst_mem);
      output_format = (memory::format)
                      sum_pd.dst_primitive_desc().desc().data.format;

      // copy back output from allocated memory
	...
	reorder_prim = reorder(*dst_mem, *target_mem);
      ...
      PADDLE_THROW("sum mkldnn kernel does not support SelectedRow yet");
    } else if (out_var->IsType<framework::LoDTensorArray>()) {
      // TODO(@mozga-intel) Add MKLDNN LoDTensorArray support
      PADDLE_THROW("sum mkldnn kernel does not support LoDTensorArray yet");      
    } else {
